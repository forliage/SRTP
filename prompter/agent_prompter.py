...
from prompter.basic import BasicPrompter
...
class AppAgentPrompter(BasicPrompter):
    ...
    def system_prompt_construction(self) -> str:
        ...
        examples = self.examples_prompt_helper(additional_examples=[])
    ...
    def user_prompt_construction(
        ...
        prompt = self.prompt_template["user"].format(
            ...
            control_item=json.dumps(control_item),
            prev_plan=json.dumps(prev_plan),
            eval_history=configs["eval_history"],
            user_request=user_request,
            retrieved_docs=retrieved_docs,
        )
    ...
    def user_content_construction(
        ...
        include_concat_screenshot: bool = True,
        step: int = 0
    ) -> List[Dict]:
        """
        Construct the prompt for LLMs.
        ...
        if self.is_visual:
            screenshot_text = []
            # The Qwen-VL context is only 6k and cannot fit three images.
            if include_last_screenshot and step > 0:
                screenshot_text += ["Screenshot for the last step, the red box annotated the control selected in the previous step:"]
            if include_concat_screenshot:
                screenshot_text += ["Current Screenshot(left) and Annotated Screenshot(right):"]
            else:
