...
class BaseService(abc.ABC):
    ...
    def get_service(name):
        ...
        if service_name:
            if name in ["aoai", "azure_ad"]:
                module = import_module(".openai", package="llm")
            else:
                module = import_module("." + name.lower(), package="llm")
    ...
    def get_cost_estimator(
        ...
        if api_type.lower() == "qwen":
            cost = (
                prompt_tokens * 0.008 / 1000
                + completion_tokens * 0.008 / 1000
            )
        elif name in prices:
            cost = (
